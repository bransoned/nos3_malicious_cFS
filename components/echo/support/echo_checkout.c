...
** Include Files
*/
#include "echo_checkout.h"


/*
** Global Variables
*/
uart_info_t EchoUart;
ECHO_Device_HK_tlm_t EchoHK;
ECHO_Device_Data_tlm_t EchoData;
...
int process_command(int cc, int num_tokens, char tokens[MAX_INPUT_TOKENS][MAX_INPUT_TOKEN_SIZE])
{
    ...
    {	
        ...
        case CMD_NOOP:
            ...
            {
                status = ECHO_CommandDevice(&EchoUart, ECHO_DEVICE_NOOP_CMD, 0);
        ...
        case CMD_HK:
            if (check_number_arguments(num_tokens, 0) == OS_SUCCESS)
            {
                status = ECHO_RequestHK(&EchoUart, &EchoHK);
                if (status == OS_SUCCESS)
                {
                    OS_printf("ECHO_RequestHK command success\n");
                }
                else
                {
                    OS_printf("ECHO_RequestHK command failed!\n");
                }
            }
            break;
        ...
        case CMD_ECHO:
            if (check_number_arguments(num_tokens, 0) == OS_SUCCESS)
            {
                status = ECHO_RequestData(&EchoUart, &EchoData);
                if (status == OS_SUCCESS)
                {
                    OS_printf("ECHO_RequestData command success\n");
                }
                else
                {
                    OS_printf("ECHO_RequestData command failed!\n");
                }
            }
            break;
        ...
        case CMD_CFG:
            if (check_number_arguments(num_tokens, 1) == OS_SUCCESS)
            {
                ...
                status = ECHO_CommandDevice(&EchoUart, ECHO_DEVICE_CFG_CMD, config);
                if (status == OS_SUCCESS)
                {
...
int main(int argc, char *argv[]) 
{
    ...
    EchoUart.deviceString = ECHO_CFG_STRING;
    EchoUart.handle = ECHO_CFG_HANDLE;
    EchoUart.isOpen = PORT_CLOSED;
    EchoUart.baud = ECHO_CFG_BAUDRATE_HZ;
    status = uart_init_port(&EchoUart);
    if (status == OS_SUCCESS)
    {
        printf("UART device %s configured with baudrate %d \n", EchoUart.deviceString, EchoUart.baud);
    }
    else
    {
        printf("UART device %s failed to initialize! \n", EchoUart.deviceString);
        run_status = OS_ERROR;
    }
    ...
    // Close the device 
    uart_close_port(&EchoUart);

    OS_printf("Cleanly exiting echo application...\n\n"); 
    return 1;
}
